...
class NeuralNetwork():
    ...
    def forward_pass(self, X) -> (List[np.ndarray], List[np.ndarray]):
        ...
        # W = number of layers, (number of neutron left * number of neutron right)
        Z = X
        #number of layers
        for i in range(self.n_layers):
            # Prepend ones to Z which Z0 = 1
            Z_with_bias = np.hstack([np.ones((Z.shape[0], 1)), Z])
            A = Z_with_bias.dot(self.W[i])  # Compute the linear combination
            Z = self.activations[i].value(A)  # Apply activation
            A_vals.append(A)
            Z_vals.append(Z)
    ...
    def backward_pass(self, A_vals, dLdyhat) -> List[np.ndarray]:
        ...
        # YOUR CODE HERE
        #####################################
        deltas = [dLdyhat * self.activations[-1].derivative(A_vals[-1])]
        for i in range(self.n_layers - 1, 0, -1):
            delta = (deltas[-1].dot(self.W[i][1:,:].T)) * self.activations[i-1].derivative(A_vals[i-1])
            deltas.append(delta)
        deltas.reverse()
        return deltas
    ...
    def update_weights(self, X, Z_vals, deltas) -> List[np.ndarray]:
        ...
        for i in range(self.n_layers):
            if i == 0:
                ...
                input_layer = Z_vals[i-1]
            # Append a column for bias term
            input_bias = np.hstack([np.ones((input_layer.shape[0], 1)), input_layer])
            # find the gradient
            # gradient = sum of gradient of all examples, so we can update weights according to all examples in the batch in one time
            gradient = input_bias.T.dot(deltas[i])
            # Update the weight
